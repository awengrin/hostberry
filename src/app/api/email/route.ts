...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await emailApi.list(domain, token);
...
export async function POST(req: NextRequest) {
  try {
    const token = await requireAuth();
    const { domain, ...userData } = await req.json();
    if (!domain) return NextResponse.json({ error: "Doména je povinná" }, { status: 400 });

    const data = await emailApi.create(domain, userData, token);
    return NextResponse.json(data);
  } catch (e) {
    if (e instanceof Error && e.message === "UNAUTHORIZED") {
      return NextResponse.json({ error: "Neautorizovaný" }, { status: 401 });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const data = await emailApi.destroy(domain, Number(userId), token);
    return NextResponse.json(data);
  } catch (e) {
    if (e instanceof Error && e.message === "UNAUTHORIZED") {
      return NextResponse.json({ error: "Neautorizovaný" }, { status: 401 });
